...
string GenerateQuery(mt19937 &generator, const vector<string> &dictionary,
    ...
    for (int i = 0; i < word_count; ++i) {
        ...
        query += dictionary[uniform_int_distribution<size_t>(0,
                dictionary.size() - 1)(generator)];
    }
    return query;
}
vector<string> GenerateQueries(mt19937 &generator,
        const vector<string> &dictionary, int query_count, int max_word_count) {
...
void Test(string mark, const SearchServer &search_server,
...
void main_test() {
    ...
    for (size_t i = 0; i < documents.size(); ++i) {
        search_server.AddDocument(static_cast<int>(i), documents[i],
